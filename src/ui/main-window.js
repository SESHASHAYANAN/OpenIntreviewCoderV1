...
class MainWindowUI {
    ...
    setupEventListeners() {
        if (window.electronAPI) {
            ...
            // Listen for mic permission denied
            if (window.electronAPI.onMicPermissionDenied) {
                window.electronAPI.onMicPermissionDenied((event, data) => {
                    this.handleRecordingStopped();
                    if (this.micButton) {
                        this.micButton.classList.add('mic-denied');
                    }
                    logger.warn('Mic permission denied', { message: data?.message });
                });
            }

            // Listen for speech errors (including permission denied)
            if (window.electronAPI.onSpeechError) {
                window.electronAPI.onSpeechError((event, data) => {
                    if (data && data.errorType === 'permission-denied') {
                        this.handleRecordingStopped();
                        if (this.micButton) {
                            this.micButton.classList.add('mic-denied');
                        }
                    }
                    logger.warn('Speech error in main window', { error: data?.error });
                });
            }
    ...
    showSettingsMenu() {
        ...
        const quitOption = this.createMenuItem('Quit openintreviewcoder', 'fa-power-off', () => {
            if (window.electronAPI) {
