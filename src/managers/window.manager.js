...
class WindowManager {
  ...
  async createWindow(type, showOnCreate = false) {
    ...
    // Windows are fixed size — no resize handler needed
  ...
  applyStealthMeasures(window, type) {
    ...
    // Make window undetectable by screen capture (Meet/Zoom/Teams screen sharing)
    try {
  ...
  // Position bound windows: main at left edge, LLM response centered in remaining space
  positionBoundWindows() {
    ...
    // Main window: pinned to left edge, vertically centered
    const mainX = displayX + leftMargin;
    const mainY = displayY + Math.round((screenHeight - mainHeight) / 2);
    mainWindow.setPosition(mainX, mainY);

    // LLM response window: centered in the remaining screen area (to the right of main)
    const remainingX = displayX + leftMargin + mainWidth + this.windowGap;
    const remainingWidth = screenWidth - mainWidth - leftMargin - this.windowGap;
    const llmX = remainingX + Math.round((remainingWidth - llmWidth) / 2);
    const llmY = displayY + Math.round((screenHeight - llmHeight) / 2);
    llmWindow.setPosition(Math.max(remainingX, llmX), llmY);

    // Update stored position
    this.boundWindowsPosition = { x: mainX, y: mainY };

    logger.debug('Positioned windows: main=left, llm=center', {
