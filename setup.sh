...
# openintreviewcoder one-shot setup: install deps, (optionally) build, and run
...
if [[ "$ENV_NEEDS_CONFIG" -eq 1 ]] || grep -q "your_gemini_api_key_here" .env 2>/dev/null; then
  ...
  echo "=========================================="
  echo ""
  echo "openintreviewcoder needs a Google Gemini API key to work."
  echo ""
